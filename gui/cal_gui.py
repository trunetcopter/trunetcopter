...
class FreeIMUCal(QMainWindow, Ui_FreeIMUCal):
  def __init__(self):
    ...
    self.connectButton.clicked.connect(self.mavlink_connect)
  ...
  def mavlink_connect(self):
    ...
    self.ser = mavutil.mavlink_connection(self.serial_port)
    print "Mavlink serial port opened correctly"
    
    self.set_status("Connected to mavlink device")
  ...
  def serial_connect(self):
    ...
    try:
      ...
      self.ser = mavutil.mavlink_connection("/dev/tty.usbserial-A8006AtT")
...
class SerialWorker(QThread):
  ...
  def run(self):
    ...
    while not self.exiting:
      msg = self.ser.recv_match(type='RAW_IMU')
      if not msg:
        continue
